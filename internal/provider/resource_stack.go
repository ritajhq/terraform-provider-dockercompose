	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"watch": {
				...
				Optional:    true,
				Computed:    true,
				Description: "Launch `docker compose up -d --watch` instead of a plain `up -d`, syncing local file changes into running containers per each service's develop.watch config. Intended for local dev only: it starts a long-running, detached watcher process and is not meant for CI/prod applies. Has no effect if no service defines develop.watch entries. Defaults to the provider-level `watch` setting; set explicitly here to override it (including explicit `false` overriding a provider default of `true`).",
			},
			"active_profiles": {
				...
				Description: "Docker Compose profiles to activate for this stack, so services tagged with matching `profiles` on the service block actually start/stop. Defaults to the provider-level `active_profiles` setting; set explicitly here (including `[]`) to override it.",
			},
			...
			"watch_pid": {
				Type:        schema.TypeInt,
				Computed:    true,
				Description: "PID of the running `docker compose --watch` process, if active. Empty/0 if watch is disabled or the watcher isn't running.",
			},
...
func serviceSchema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		// --- Develop (Compose Watch) ---
		"develop_watch": {
			Type:        schema.TypeList,
			Optional:    true,
			Description: "Compose Watch entries (develop.watch) for syncing local file changes into this service's container. Only takes effect when the resource's top-level watch = true.",
			Elem: &schema.Resource{
				Schema: developWatchSchema(),
			},
		},
	}
}

func developWatchSchema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		"path": {
			Type:        schema.TypeString,
			Required:    true,
			Description: "Host path to watch for changes.",
		},
		"action": {
			Type:        schema.TypeString,
			Required:    true,
			Description: "Action to take on change: sync, rebuild, sync+restart, or sync+exec.",
			ValidateFunc: func(v interface{}, k string) ([]string, []error) {
				value := v.(string)
				for _, allowed := range []string{"sync", "rebuild", "sync+restart", "sync+exec"} {
					if value == allowed {
						return nil, nil
					}
				}
				return nil, []error{fmt.Errorf("%s must be one of sync, rebuild, sync+restart, sync+exec, got %q", k, value)}
			},
		},
		"target": {
			Type:        schema.TypeString,
			Optional:    true,
			Description: "Container path to sync into.",
		},
		"ignore": {
			Type:        schema.TypeList,
			Optional:    true,
			Elem:        &schema.Schema{Type: schema.TypeString},
			Description: "Paths to ignore within the watched path.",
		},
		"exec_command": {
			Type:        schema.TypeString,
			Optional:    true,
			Description: "Command to run inside the container after sync (only used with action = sync+exec).",
		},
	}
}
...
func resourceStackCreate(d *schema.ResourceData, m interface{}) error {
	...
	// Resolve watch/active_profiles: resource-level value wins if explicitly set,
	// otherwise fall back to the provider-level default.
	watchRequested := resolveBool(d, "watch", client.DefaultWatch)
	activeProfiles := resolveStringList(d, "active_profiles", client.DefaultActiveProfiles)

	// Run docker compose up, launching the detached watcher instead when requested
	// and at least one service defines develop.watch entries.
	pid, err := startOrPlainUp(client, stackName, composeFilePath, watchRequested, cf, activeProfiles)
	if err != nil {
		return err
	}
	...
	if err := d.Set("watch", watchRequested); err != nil {
		return fmt.Errorf("error setting watch: %s", err)
	}
	if err := d.Set("active_profiles", activeProfiles); err != nil {
		return fmt.Errorf("error setting active_profiles: %s", err)
	}
	if err := setWatchPID(d, pid); err != nil {
		return err
	}
...
func resourceStackRead(d *schema.ResourceData, m interface{}) error {
	...
	// Resolved active_profiles is stored in state from the last Create/Update; Read
	// doesn't re-resolve against the provider default since it has no config diff to
	// apply, only the previously-applied set to reflect.
	activeProfiles := getStrListValue(d.Get("active_profiles").([]interface{}))

	// Verify services are running (restricted to the active profile set)
	output, err := client.ComposePSServicesProfiles(stackName, composeFilePath, activeProfiles)
	...
	// Check whether the watcher (if any) is still alive, clearing watch_pid if not.
	if err := reconcileWatchOnRead(d, composeFilePath); err != nil {
		return err
	}

	// Read container runtime info (IDs, IPs, ports, health, etc.), restricted to the
	// active profile set so it only reflects currently-active, profile-included services.
	return readContainerInfo(d, client, stackName, composeFilePath, activeProfiles...)
}
...
func resourceStackUpdate(d *schema.ResourceData, m interface{}) error {
	...
	// otherwise fall back to the provider-level default.
	watchRequested := resolveBool(d, "watch", client.DefaultWatch)
	newProfiles := resolveStringList(d, "active_profiles", client.DefaultActiveProfiles)

	// Diff the previously-applied active_profiles (stored in state) against the newly
	// resolved value: services whose profile dropped out are stopped (not removed)
	// before re-applying the compose file with the new profile set.
	oldProfilesRaw, _ := d.GetChange("active_profiles")
	oldProfiles := getStrListValue(oldProfilesRaw.([]interface{}))
	if err := stopDroppedProfileServices(client, stackName, composeFilePath, cf, oldProfiles, newProfiles); err != nil {
		return err
	}

	// Apply the compose diff, then always kill and respawn the watcher if one is
	// (or should be) running — regardless of whether the diff itself required it.
	pid, err := reconcileWatchOnUpdate(client, stackName, composeFilePath, watchRequested, cf, newProfiles)
	if err != nil {
		return err
	}
	...
	if err := d.Set("active_profiles", newProfiles); err != nil {
		return fmt.Errorf("error setting active_profiles: %s", err)
	}
	if err := setWatchPID(d, pid); err != nil {
		return err
...
func resourceStackDelete(d *schema.ResourceData, m interface{}) error {
	...
	// Stop any running watcher before tearing down the stack. If its PID is already
	// dead this just proceeds (destroy must not fail because the watcher already exited).
	if err := stopWatchOnDelete(composeFilePath); err != nil {
		return fmt.Errorf("error stopping watcher: %s", err)
	}
...
func buildComposeFile(d *schema.ResourceData) *docker.ComposeFile {
	...
	for _, raw := range rawServices {
		...
		// Build develop.watch config if any watch entries are set
		if rawWatch, ok := svc["develop_watch"].([]interface{}); ok && len(rawWatch) > 0 {
			watches := make([]docker.WatchConfig, 0, len(rawWatch))
			for _, rw := range rawWatch {
				w := rw.(map[string]interface{})
				watch := docker.WatchConfig{
					Path:   getStr(w, "path"),
					Action: getStr(w, "action"),
					Target: getStr(w, "target"),
					Ignore: getStrList(w, "ignore"),
				}
				if execCmd := getStr(w, "exec_command"); execCmd != "" {
					watch.Exec = &docker.WatchExec{Command: execCmd}
				}
				watches = append(watches, watch)
			}
			config.Develop = &docker.DevelopConfig{Watch: watches}
		}
