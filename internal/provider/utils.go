...
// getStrListValue converts a raw []interface{} (as returned directly by d.Get on a
// TypeList of strings) into a []string, skipping non-string/empty entries.
func getStrListValue(raw []interface{}) []string {
	result := make([]string, 0, len(raw))
	for _, v := range raw {
