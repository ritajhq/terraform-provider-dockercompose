...
func TestProviderSchema(t *testing.T) {
	...
	expectedFields := []string{"host", "docker_binary", "project_directory", "watch", "active_profiles"}
...
func TestProviderSchemaWatchAndActiveProfiles(t *testing.T) {
	p := Provider()

	watch := p.Schema["watch"]
	if watch.Type != schema.TypeBool {
		t.Errorf("provider watch should be TypeBool, got %v", watch.Type)
	}
	if watch.Required {
		t.Error("provider watch should be Optional")
	}

	profiles := p.Schema["active_profiles"]
	if profiles.Type != schema.TypeList {
		t.Errorf("provider active_profiles should be TypeList, got %v", profiles.Type)
	}
	if profiles.Required {
		t.Error("provider active_profiles should be Optional")
	}
}
...
func TestStackWatchIsThreeState(t *testing.T) {
	r := resourceComposeStack()
	watch, ok := r.Schema["watch"]
	if !ok {
		t.Fatal("stack schema missing 'watch' field")
	}
	if !watch.Optional {
		t.Error("stack 'watch' should be Optional")
	}
	if !watch.Computed {
		t.Error("stack 'watch' should be Computed, so unset can be distinguished from explicit false via GetRawConfig")
	}
	if watch.Default != nil {
		t.Error("stack 'watch' should not set Default now that it's Optional+Computed and resolved against the provider default in code")
	}
}

func TestStackActiveProfilesSchema(t *testing.T) {
	r := resourceComposeStack()
	profiles, ok := r.Schema["active_profiles"]
	if !ok {
		t.Fatal("stack schema missing 'active_profiles' field")
	}
	if profiles.Type != schema.TypeList {
		t.Errorf("stack 'active_profiles' should be TypeList, got %v", profiles.Type)
	}
	if !profiles.Optional {
		t.Error("stack 'active_profiles' should be Optional")
	}
	if !profiles.Computed {
		t.Error("stack 'active_profiles' should be Computed so the resolved value can be stored for Update diffing")
	}
}
