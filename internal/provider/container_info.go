...
// profiles, if given, restricts the query to those active Compose profiles so the
// computed container list only reflects currently-active, profile-included services.
func readContainerInfo(d *schema.ResourceData, client *docker.DockerClient, stackName, composeFilePath string, profiles ...string) error {
	// Get container list via docker compose ps --format json
	psJSON, err := client.ComposePSJSONProfiles(stackName, composeFilePath, profiles)
	if err != nil {
