...
func resourceProjectCreate(d *schema.ResourceData, m interface{}) error {
	...
	cf, err := loadProjectComposeFile(composeFilePath)
	if err != nil {
		return err
	}

	watchRequested := d.Get("watch").(bool)
	pid, err := startOrPlainUp(client, stackName, composeFilePath, watchRequested, cf, nil)
	if err != nil {
...
func resourceProjectUpdate(d *schema.ResourceData, m interface{}) error {
	...
	watchRequested := d.Get("watch").(bool)
	pid, err := reconcileWatchOnUpdate(client, stackName, composeFilePath, watchRequested, cf, nil)
	if err != nil {
...
func resourceProjectDelete(d *schema.ResourceData, m interface{}) error {
	...
	// Stop any running watcher before tearing down the project. If its PID is already
...
// loadProjectComposeFile reads and parses composeFilePath so develop.watch entries
// can be detected. dockercompose_project sources services from raw/inline YAML rather
// than HCL blocks, so this is the only way to know whether watch entries exist.
func loadProjectComposeFile(composeFilePath string) (*docker.ComposeFile, error) {
	content, err := os.ReadFile(composeFilePath)
	if err != nil {
		return nil, fmt.Errorf("error reading compose file: %s", err)
	}
	cf, err := docker.UnmarshalComposeFile(content)
	if err != nil {
		return nil, fmt.Errorf("error parsing compose file: %s", err)
	}
	return cf, nil
}
