...
func TestMarshalOmitsEmptyFields(t *testing.T) {
	...
	omitted := []string{
		...
		"mem_limit:", "mem_reservation:", "cpus:", "develop:",
...
func TestMarshalDevelopWatch(t *testing.T) {
	cf := &ComposeFile{
		Services: map[string]*ServiceConfig{
			"web": {
				Image: "myapp:latest",
				Develop: &DevelopConfig{
					Watch: []WatchConfig{
						...
						},
						{
							...
							Action: "sync+exec",
							Exec:   &WatchExec{Command: "app reload"},
						},
					},
				},
			},
	...
	yaml := string(yamlBytes)
	assertContains(t, yaml, "develop:")
	assertContains(t, yaml, "watch:")
	assertContains(t, yaml, "path: ./src")
	...
	assertContains(t, yaml, "exec:")
	assertContains(t, yaml, "command: app reload")
}

func TestMarshalOmitsDevelopWhenAbsent(t *testing.T) {
	cf := &ComposeFile{
		Services: map[string]*ServiceConfig{
			"web": {Image: "nginx:latest"},
		},
	...
	yaml := string(yamlBytes)
	assertNotContains(t, yaml, "develop:")
	assertNotContains(t, yaml, "watch:")
}

func TestUnmarshalDevelopWatch(t *testing.T) {
	yamlData := `
services:
  web:
    image: myapp:latest
...
`
	...
	if web.Develop == nil {
		t.Fatal("web.Develop not parsed")
	}
	if len(web.Develop.Watch) != 2 {
		t.Fatalf("expected 2 watch entries, got %d", len(web.Develop.Watch))
	}

	first := web.Develop.Watch[0]
	if first.Path != "./src" || first.Action != "sync" || first.Target != "/app/src" {
		t.Errorf("first watch entry = %+v, want path=./src action=sync target=/app/src", first)
	}
	if len(first.Ignore) != 1 || first.Ignore[0] != "node_modules/" {
		t.Errorf("first.Ignore = %v, want [node_modules/]", first.Ignore)
	}

	second := web.Develop.Watch[1]
	if second.Action != "sync+exec" {
		t.Errorf("second.Action = %q, want sync+exec", second.Action)
	}
	if second.Exec == nil || second.Exec.Command != "app reload" {
		t.Errorf("second.Exec = %+v, want Command=app reload", second.Exec)
	}
}

func TestMarshalUnmarshalDevelopWatchRoundTrip(t *testing.T) {
	original := &ComposeFile{
		Services: map[string]*ServiceConfig{
			"api": {
				Image: "myapp:v1.0",
	...
	api := restored.Services["api"]
	if api == nil {
		t.Fatal("service 'api' not found after round-trip")
	}
	if api.Develop == nil || len(api.Develop.Watch) != 2 {
		t.Fatalf("Develop.Watch not preserved: %+v", api.Develop)
	}
	if api.Develop.Watch[0].Path != "./src" || api.Develop.Watch[0].Action != "sync" {
		t.Errorf("Watch[0] = %+v, want path=./src action=sync", api.Develop.Watch[0])
	}
	if len(api.Develop.Watch[0].Ignore) != 1 || api.Develop.Watch[0].Ignore[0] != "*.tmp" {
		t.Errorf("Watch[0].Ignore = %v, want [*.tmp]", api.Develop.Watch[0].Ignore)
	}
	if api.Develop.Watch[1].Exec == nil || api.Develop.Watch[1].Exec.Command != "app reload" {
		t.Errorf("Watch[1].Exec = %+v, want Command=app reload", api.Develop.Watch[1].Exec)
	}
}
