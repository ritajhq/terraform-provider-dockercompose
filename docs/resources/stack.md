---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "dockercompose_stack Resource - dockercompose"
subcategory: ""
description: |-
  
---

# dockercompose_stack (Resource)



## Example Usage

```terraform
terraform {
  required_providers {
    dockercompose = {
      source  = "ritajhq/dockercompose"
      version = "~> 1.0"
    }
  }
}

resource "dockercompose_stack" "monitoring" {
  name = "metrics-stack"

  # Clean up associated volumes strictly on destroy
  remove_volumes_on_destroy = true

  # Primary database service
  service {
    name           = "database"
    image          = "postgres:15-alpine"
    container_name = "monitoring_db"
    restart        = "unless-stopped"

    environment = {
      POSTGRES_USER     = "admin"
      POSTGRES_PASSWORD = "secretpassword"
      POSTGRES_DB       = "metrics"
    }

    # Restrict Hardware resources
    resource_limits_memory = "512M"
    resource_limits_cpus   = "1.0"

    # Volume mounts are defined as strings (host:container or named:container)
    volumes = ["db_data:/var/lib/postgresql/data"]
    
    ports = ["5432:5432"]

    # Healthcheck attributes
    healthcheck_test     = ["CMD", "pg_isready", "-U", "admin"]
    healthcheck_interval = "10s"
    healthcheck_timeout  = "5s"
    healthcheck_retries  = 5
    
    networks = ["monitoring_net"]
  }

  # Web application
  service {
    name           = "grafana"
    image          = "grafana/grafana:latest"
    container_name = "monitoring_ui"
    depends_on     = ["database"]
    ports          = ["3000:3000"]
    
    volumes  = ["grafana_data:/var/lib/grafana"]
    networks = ["monitoring_net"]
  }

  # Register volume definitions
  volume {
    name = "db_data"
  }

  volume {
    name = "grafana_data"
  }

  # Register network definitions
  network {
    name = "monitoring_net"
  }
}

# Toggle this to enable "docker compose up --watch" for local dev, leave it
# false everywhere else (staging/prod/CI). watch is intended for local dev
# only: it starts a long-running, detached watcher process attached to this
# stack and is not meant for CI/prod applies.
variable "enable_watch" {
  type    = bool
  default = false
}

resource "dockercompose_stack" "webapp" {
  name  = "webapp-dev"
  watch = var.enable_watch

  service {
    name  = "web"
    image = "myapp:dev"
    ports = ["3000:3000"]

    volumes = ["./src:/app/src"]

    # develop.watch entries only take effect when watch = true above. With
    # watch = false, the exact same config runs as a plain `docker compose up -d`.
    develop_watch {
      path   = "./src"
      action = "sync"
      target = "/app/src"
      ignore = ["node_modules/", "*.log"]
    }

    develop_watch {
      path   = "./package.json"
      action = "rebuild"
    }
  }
}

# --- Provider-level defaults + per-resource overrides ---
#
# `watch` and `active_profiles` can both be set once on the provider and
# inherited by every dockercompose_stack, or overridden per-stack. The
# resource-level value always wins when explicitly set — including an
# explicit `false` or `[]` overriding a non-default provider setting.
provider "dockercompose" {
  alias = "dev"

  watch           = true
  active_profiles = ["debug"]
}

resource "dockercompose_stack" "inherits_provider_defaults" {
  provider = dockercompose.dev
  name     = "webapp-dev-defaults"

  # Neither `watch` nor `active_profiles` is set here, so this stack inherits
  # watch = true and active_profiles = ["debug"] from the provider above.
  service {
    name  = "web"
    image = "myapp:dev"
    ports = ["3001:3000"]

    develop_watch {
      path   = "./src"
      action = "sync"
      target = "/app/src"
    }
  }

  # Only starts because the "debug" profile is active (inherited from the provider).
  service {
    name     = "debugger"
    image    = "myapp:debug-tools"
    profiles = ["debug"]
  }
}

resource "dockercompose_stack" "overrides_provider_defaults" {
  provider = dockercompose.dev

  name = "webapp-ci"

  # Explicitly override both provider defaults for this stack: no watcher, no
  # profiles active, even though the provider default is watch = true and
  # active_profiles = ["debug"].
  watch           = false
  active_profiles = []

  service {
    name  = "web"
    image = "myapp:dev"
    ports = ["3002:3000"]
  }

  # Stays stopped here since active_profiles = [] overrides the provider default.
  service {
    name     = "debugger"
    image    = "myapp:debug-tools"
    profiles = ["debug"]
  }
}
```

<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `name` (String) Project name for the Docker Compose stack. Used as the -p flag.
- `service` (Block List, Min: 1) Service definitions. (see [below for nested schema](#nestedblock--service))

### Optional

- `active_profiles` (List of String) Docker Compose profiles to activate for this stack, so services tagged with matching `profiles` on the service block actually start/stop. Defaults to the provider-level `active_profiles` setting; set explicitly here (including `[]`) to override it.
- `config` (Block List) Config definitions (Docker configs). (see [below for nested schema](#nestedblock--config))
- `network` (Block List) Network definitions. (see [below for nested schema](#nestedblock--network))
- `remove_volumes_on_destroy` (Boolean) Whether to remove named volumes on destroy (docker compose down -v).
- `secret` (Block List) Secret definitions (Docker secrets). (see [below for nested schema](#nestedblock--secret))
- `volume` (Block List) Volume definitions. (see [below for nested schema](#nestedblock--volume))
- `watch` (Boolean) Launch `docker compose up -d --watch` instead of a plain `up -d`, syncing local file changes into running containers per each service's develop.watch config. Intended for local dev only: it starts a long-running, detached watcher process and is not meant for CI/prod applies. Has no effect if no service defines develop.watch entries. Defaults to the provider-level `watch` setting; set explicitly here to override it (including explicit `false` overriding a provider default of `true`).
- `working_dir` (String) Working directory for the stack. If not set, uses <project_directory>/<name>/.

### Read-Only

- `compose_file_path` (String) Path to the generated docker-compose.yml file.
- `compose_yaml` (String) The generated docker-compose YAML content.
- `container` (List of Object) Runtime information about containers in the stack (populated after apply). Sorted by service name. (see [below for nested schema](#nestedatt--container))
- `id` (String) The ID of this resource.
- `watch_pid` (Number) PID of the running `docker compose --watch` process, if active. Empty/0 if watch is disabled or the watcher isn't running.

<a id="nestedblock--service"></a>
### Nested Schema for `service`

Required:

- `image` (String) Docker image.
- `name` (String) Service name.

Optional:

- `cap_add` (List of String) Add Linux capabilities.
- `cap_drop` (List of String) Drop Linux capabilities.
- `command` (List of String) Override default command.
- `container_name` (String) Custom container name.
- `cpus` (String) CPU limit (legacy, prefer resource_limits_cpus).
- `depends_on` (List of String) Service dependencies.
- `develop_watch` (Block List) Compose Watch entries (develop.watch) for syncing local file changes into this service's container. Only takes effect when the resource's top-level watch = true. (see [below for nested schema](#nestedblock--service--develop_watch))
- `devices` (List of String) Device mappings.
- `dns` (List of String) Custom DNS servers.
- `domainname` (String) Container domain name.
- `entrypoint` (List of String) Override default entrypoint.
- `env_file` (List of String) Env file paths.
- `environment` (Map of String) Environment variables.
- `expose` (List of String) Expose ports without publishing to host.
- `extra_hosts` (List of String) Extra /etc/hosts entries (hostname:ip).
- `group_add` (List of String) Additional groups the container process should join.
- `healthcheck_disable` (Boolean) Disable healthcheck.
- `healthcheck_interval` (String) Healthcheck interval (e.g. '30s').
- `healthcheck_retries` (Number) Healthcheck max retries.
- `healthcheck_start_period` (String) Healthcheck start period.
- `healthcheck_test` (List of String) Healthcheck command (e.g. ['CMD', 'curl', '-f', 'http://localhost']).
- `healthcheck_timeout` (String) Healthcheck timeout.
- `hostname` (String) Container hostname.
- `init` (Boolean) Run init process inside the container.
- `ipc` (String) IPC mode.
- `labels` (Map of String) Container labels.
- `logging_driver` (String) Logging driver (json-file, syslog, etc.).
- `logging_options` (Map of String) Logging driver options.
- `mem_limit` (String) Memory limit (legacy, prefer resource_limits_memory).
- `mem_reservation` (String) Memory reservation (legacy).
- `network_mode` (String) Network mode (bridge, host, none, service:name).
- `networks` (List of String) Networks to attach to.
- `pid` (String) PID mode.
- `platform` (String) Target platform (e.g. linux/amd64).
- `ports` (List of String) Port mappings (host:container).
- `privileged` (Boolean) Run in privileged mode.
- `profiles` (List of String) Profiles this service belongs to.
- `pull_policy` (String) Image pull policy (always, never, missing, build).
- `read_only` (Boolean) Mount root filesystem as read-only.
- `replicas` (Number) Number of container replicas.
- `resource_limits_cpus` (String) CPU limit (e.g. '0.5').
- `resource_limits_memory` (String) Memory limit (e.g. '512M').
- `resource_reservations_cpus` (String) CPU reservation.
- `resource_reservations_memory` (String) Memory reservation.
- `restart` (String) Restart policy: no, always, on-failure, unless-stopped.
- `runtime` (String) Runtime (e.g. runc, nvidia).
- `security_opt` (List of String) Security options.
- `shm_size` (String) Size of /dev/shm.
- `stdin_open` (Boolean) Keep stdin open (docker run -i).
- `stop_grace_period` (String) Time to wait before force-stopping.
- `stop_signal` (String) Signal to stop the container.
- `sysctls` (Map of String) Sysctl settings.
- `tmpfs` (List of String) Tmpfs mounts.
- `tty` (Boolean) Allocate pseudo-TTY (docker run -t).
- `user` (String) User to run as (user:group).
- `volumes` (List of String) Volume mounts (host:container or named:container).
- `working_dir` (String) Working directory inside the container.

<a id="nestedblock--service--develop_watch"></a>
### Nested Schema for `service.develop_watch`

Required:

- `action` (String) Action to take on change: sync, rebuild, sync+restart, or sync+exec.
- `path` (String) Host path to watch for changes.

Optional:

- `exec_command` (String) Command to run inside the container after sync (only used with action = sync+exec).
- `ignore` (List of String) Paths to ignore within the watched path.
- `target` (String) Container path to sync into.


<a id="nestedblock--config"></a>
### Nested Schema for `config`

Required:

- `file` (String) Path to the config file.
- `name` (String) Config name.


<a id="nestedblock--network"></a>
### Nested Schema for `network`

Required:

- `name` (String) Network key within the compose file.

Optional:

- `attachable` (Boolean) Allow manual container attachment.
- `driver` (String) Network driver (bridge, overlay, host, none).
- `driver_opts` (Map of String) Driver-specific options.
- `external` (Boolean) Use externally created network. Implied automatically when external_name is set.
- `external_name` (String) Literal Docker network name to use (Compose's top-level `name:` field), bypassing the `<project>_` prefix. Set this to the `name` of a dockercompose_network resource or dockercompose_network data source when joining a network shared across stacks or created outside this stack. Implies external = true — no need to set that separately.
- `internal` (Boolean) Restrict external access.
- `ipam_driver` (String) IPAM driver.
- `ipam_gateway` (String) IPAM gateway (e.g. '172.28.0.1').
- `ipam_subnet` (String) IPAM subnet (e.g. '172.28.0.0/16').
- `labels` (Map of String) Network labels.


<a id="nestedblock--secret"></a>
### Nested Schema for `secret`

Required:

- `file` (String, Sensitive) Path to the secret file.
- `name` (String) Secret name.


<a id="nestedblock--volume"></a>
### Nested Schema for `volume`

Required:

- `name` (String) Volume name.

Optional:

- `driver` (String) Volume driver.
- `driver_opts` (Map of String) Driver-specific options.
- `external` (Boolean) Use externally created volume.
- `labels` (Map of String) Volume labels.


<a id="nestedatt--container"></a>
### Nested Schema for `container`

Read-Only:

- `container_id` (String)
- `container_name` (String)
- `exit_code` (Number)
- `health` (String)
- `image` (String)
- `ip_address` (String)
- `network_settings` (List of Object) (see [below for nested schema](#nestedobjatt--container--network_settings))
- `ports` (List of Object) (see [below for nested schema](#nestedobjatt--container--ports))
- `service` (String)
- `state` (String)

<a id="nestedobjatt--container--network_settings"></a>
### Nested Schema for `container.network_settings`

Read-Only:

- `gateway` (String)
- `ip_address` (String)
- `mac_address` (String)
- `name` (String)


<a id="nestedobjatt--container--ports"></a>
### Nested Schema for `container.ports`

Read-Only:

- `ip` (String)
- `private_port` (Number)
- `protocol` (String)
- `public_port` (Number)
